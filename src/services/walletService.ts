...
// Reads the logged-in user's _id from the 'userInfo' entry and prefixes all
// per-user cache keys so that different accounts never collide.
// ---------------------------------------------------------------------------
...
export const getLocalBalanceAdjustment = (): number => {
    try { return parseFloat(localStorage.getItem(userKey('dw_local_bal_adj')) || '0') || 0; } catch { return 0; }
};

export const applyLocalBalanceChange = (delta: number): void => {
    try {
        const current = getLocalBalanceAdjustment();
        localStorage.setItem(userKey('dw_local_bal_adj'), (current + delta).toFixed(8));
    } catch { /* ignore */ }
};

export const resetLocalBalanceAdjustment = (): void => {
    try { localStorage.removeItem(userKey('dw_local_bal_adj')); } catch { /* ignore */ }
};
...
export const runBalanceAdjMigrationIfNeeded = (): void => {
    try {
        if (localStorage.getItem(userKey('dw_adj_v2_migrated'))) return; // already done
        const futuresBal = parseFloat(localStorage.getItem(userKey('dw_futures_balance')) || '0') || 0;
        // Only migrate if there IS a futures balance; if 0 the adj should also be 0
        if (futuresBal > 0) {
            localStorage.setItem(userKey('dw_local_bal_adj'), (-futuresBal).toFixed(8));
        } else {
            localStorage.removeItem(userKey('dw_local_bal_adj'));
        }
        localStorage.setItem(userKey('dw_adj_v2_migrated'), '1');
    } catch { /* ignore */ }
};
// ---------------------------------------------------------------------------
...
export const getCachedBalance = (): number => {
    try { return parseFloat(localStorage.getItem(userKey('dw_cached_balance')) || '0') || 0; } catch { return 0; }
};

export const setCachedBalance = (balance: number): void => {
    try { localStorage.setItem(userKey('dw_cached_balance'), balance.toFixed(8)); } catch { /* ignore */ }
};
...
export const getSpotHoldings = (): Record<string, number> => {
    try {
        const raw = localStorage.getItem(userKey('dw_spot_holdings'));
...
export const getSpotTimestamps = (): Record<string, number> => {
    try { return JSON.parse(localStorage.getItem(userKey('dw_spot_timestamps')) || '{}'); } catch { return {}; }
};

const saveSpotHoldings = (holdings: Record<string, number>): void => {
    try { localStorage.setItem(userKey('dw_spot_holdings'), JSON.stringify(holdings)); } catch { /* ignore */ }
};
...
export const addSpotHolding = (coin: string, amount: number, usdtCost?: number): void => {
    ...
    try {
        const tsKey = userKey('dw_spot_timestamps');
    ...
    if (usdtCost && usdtCost > 0) {
        try {
            const costKey = userKey('dw_cost_basis');
...
export const getSpotCostBasis = (): Record<string, number> => {
    try { return JSON.parse(localStorage.getItem(userKey('dw_cost_basis')) || '{}'); } catch { return {}; }
};
...
export const getFuturesBalance = (): number => {
    try { return parseFloat(localStorage.getItem(userKey('dw_futures_balance')) || '0') || 0; } catch { return 0; }
};

export const setFuturesBalance = (balance: number): void => {
    try { localStorage.setItem(userKey('dw_futures_balance'), balance.toFixed(8)); } catch { /* ignore */ }
};
